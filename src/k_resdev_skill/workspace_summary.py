...
def generate_workspace_summary(
    ...
    profile_pack_package = (
        generate_profile_pack_investigation_package(workspace)
    ...
    summary = WorkspaceSummaryResult(
        ...
        profile_pack_package_status=profile_pack_package.status if profile_pack_package else None,
        ...
        admin_profile_pack_review_status=admin_profile_pack_review.status if admin_profile_pack_review else None,
...
def render_workspace_summary_markdown(summary: WorkspaceSummaryResult) -> str:
    lines = [
        ...
        f"| Profile promotion revoke plan | {_escape(summary.profile_promotion_revoke_status or '-')} |",
...
def _load_profile_revoke_plan(workspace: Path):
...
def _load_profile_revoke_result(workspace: Path):
