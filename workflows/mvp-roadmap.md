...
Status: `national-rnd-basic` is present as a generic needs-review template skeleton only. `iris-innopolis-2026-017795` is present as the first narrow official-source-backed profile seed, but it also remains `needs_review` unless a workspace owner runs the guarded promotion flow. A profile registry/validator, profile-source record shell, profile-source review queue, profile-source fix plans, profile-integrity checker, profile-promotion review, hash-bound human promotion records, non-destructive profile-promotion apply plans, guarded apply results/backups, and generic budget evidence checklist are implemented so local profiles can be inspected and promoted without hardcoding unverified rules.
...
Status: trace passport/checkpoint ledger, artifact authority levels, goals/deadline review, local weekly review/workspace dashboard, thin workflow router, reference corpus adapters, read-only workspace discovery/setup proposals, profile-source review queues, profile-source fix plans, profile-review outputs, profile-promotion summaries, non-destructive profile-promotion apply plans, guarded profile-promotion apply results, non-destructive profile-promotion revocation plans, guarded profile-promotion revoke results, and profile lifecycle ledgers are implemented as local first passes. Artifact authority labels now keep raw sources, extracted candidates, evidence states, draft projections, approved projections, operating summaries, rejected artifacts, and superseded artifacts visibly separated across doctor, summary, review-pack, and trace flows. Goals/deadline review links local objectives and deadlines to KPIs, milestones, evidence, reports, and approvals without hardcoding official agency schedules. Weekly reviews, dashboards, workflow plans, profile-source queues, profile-source fix plans, profile-review outputs, promotion summaries, apply plans, apply results, revoke plans, revoke results, and lifecycle ledgers summarize local artifacts only and remain operating projections. The next specialization work should add supplied human review records for profile-source fix actions before expanding agency-specific packs.
